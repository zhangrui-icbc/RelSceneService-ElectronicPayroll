...
$(function() {
	...
	function  getSalaryInfo(){
		 $.ajax({
			    url : ctx+"/salary/getSalaryInfo",// 获取自己系统后台用户信息接口
			    data :{},
			    type : "POST",
			    dataType: "json",
...
//						 	      支出合计
	...
	function jumpDetail(salaryId,issueTime,userId){
		window.location.href = ctx+"/salaryWebUser/jumpDetail?salaryId="+salaryId+"&&issueTime="+issueTime+"&&userId="+userId;
	}
	/**
	 * 汇总页面
	 * @returns
	...
	function jumpSummery(){
		window.location.href = ctx+"/salaryWebUser/summary";
	}
	...
	//这里为用ajax获取用户信息并进行验证，如果账户密码不匹配则登录失败，如不需要验证用户信息，这段可不写
	 $.ajax({
	    url : ctx+"/salaryWebUser/login",// 获取自己系统后台用户信息接口
