...
$("#mytab_04").click(function(){
	$.ajax({
	    url : ctx+"/reimbursement/upLoadLog",// 获取自己系统后台用户信息接口
	    type : "POST",
...
$.ajax({
    url : ctx+"/reimbursement/delLog",// 获取自己系统后台用户信息接口
    type : "POST",
    data:{"reId":id},
    success : function(res) {
    	if(res.code=="0"){
...
$(".com").click(function(){
		$.ajax({
	    url : ctx+"/reCommon/getCommonTemplate",// 获取自己系统后台用户信息接口
	    type : "POST",
	    success : function(res) {
	    	if(res.code==0){
	    		var str = "";
...
function  getCustom(){
		$.ajax({
	    url : ctx+"/reCustom/getCustomTemplate",// 获取自己系统后台用户信息接口
	    type : "POST",
	    success : function(res) {
	    	if(res.code=="0"){
        		layerMsg(res.msg);
...
function delCom(id){
		$.ajax({
	    url : ctx+"/reCommon/delCommonTemplate?id="+id,// 获取自己系统后台用户信息接口
	    success : function(res) {
	    	if(res.code==0){
	    		if(res.code=="0"){
	        		layerMsg(res.msg);
...
function delAlter(id){
	...
	$.ajax({
    url : ctx+"/reAlternative/delAlternative?id="+id,// 获取自己系统后台用户信息接口
    success : function(res) {
    	if(res.code=="0"){
    		layerMsg(res.msg);
    	}else if(res.code=="500"){
...
$(".alter").click(function(){
		$.ajax({
	    url : ctx+"/reAlternative/alternative",// 获取自己系统后台用户信息接口
	    type : "POST",
	    success : function(res) {
	    	if(res.code==0){
	    		var str = "";
...
function addAlter(){
	   ...
	   var temp ={"type":type,"name":name,"category":category}
		$.ajax({
	    url : ctx+"/reAlternative/addAlternative",// 获取自己系统后台用户信息接口
	    type : "POST",
...
function updateAlter(arr){
	   ...
	   var temp ={"id":id,"name":column,"category":column_num}
		$.ajax({
	    url : ctx+"/reAlternative/updateAlternative",// 获取自己系统后台用户信息接口
	    type : "POST",
	    data:JSON.stringify(temp),
	    contentType:"application/json",
	    dataType: "json",
...
function  defineCom(){
   	   ...
   	   list[0]=temp;
		$.ajax({
	    url : ctx+"/reCommon/defineCommonTemplate",// 获取自己系统后台用户信息接口
...
function  updateCom(arr){
	   var id=arr[0];
		...
		$.ajax({
	    url : ctx+"/reCommon/updateCommonTemplate",// 获取自己系统后台用户信息接口
	    data :JSON.stringify(temp),
	    type : "POST",
	    contentType:"application/json",
	    dataType: "json",
...
$(".gz-download").click(function(){
	window.location.href=ctx+"/reimbursement/export";
})
