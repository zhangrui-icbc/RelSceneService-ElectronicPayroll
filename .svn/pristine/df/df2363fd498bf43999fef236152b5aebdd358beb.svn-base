...
$("#mytab_04").click(function(){
	$.ajax({
	    url : ctx+"/salary/upLoadLog",// 获取自己系统后台用户信息接口
	    type : "POST",
	    success : function(res) {
	    	if(res.code==0){
	    		...
	    		for(var i=0;i<list.length;i++){
	    			var delHtml = '<button id="bElim" type="button" class="iconfont icon-shanchu btn btn-sm btn-default">' +
...
$.ajax({
    url : ctx+"/salary/delLog",// 获取自己系统后台用户信息接口
    type : "POST",
    data:{"salaryId":id},
    success : function(res) {
    	if(res.code=="0"){
...
$(".com").click(function(){
		$.ajax({
	    url : ctx+"/common/getCommonTemplate",// 获取自己系统后台用户信息接口
	    type : "POST",
	    success : function(res) {
	    	if(res.code==0){
	    		var str = "";
...
function  getCustom(){
		$.ajax({
	    url : ctx+"/custom/getCustomTemplate",// 获取自己系统后台用户信息接口
	    type : "POST",
	    success : function(res) {
	    	if(res.code=="0"){
        		layerMsg(res.msg);
...
function delCom(id){
		$.ajax({
	    url : ctx+"/common/delCommonTemplate?id="+id,// 获取自己系统后台用户信息接口
	    success : function(res) {
	    	if(res.code==0){
	    		if(res.code=="0"){
	        		layerMsg(res.msg);
...
function delAlter(id){
	...
	$.ajax({
    url : ctx+"/alternative/delAlternative?id="+id,// 获取自己系统后台用户信息接口
    success : function(res) {
    	if(res.code=="0"){
    		layerMsg(res.msg);
    	}else if(res.code=="500"){
...
$(".alter").click(function(){
		$.ajax({
	    url : ctx+"/alternative/alternative",// 获取自己系统后台用户信息接口
	    type : "POST",
	    success : function(res) {
	    	if(res.code==0){
	    		var str = "";
...
function addAlter(){
	   ...
	   var temp ={"type":type,"name":name,"category":category}
		$.ajax({
	    url : ctx+"/alternative/addAlternative",// 获取自己系统后台用户信息接口
	    type : "POST",
...
function updateAlter(arr){
	   ...
	   var temp ={"id":id,"name":column,"category":column_num}
		$.ajax({
	    url : ctx+"/alternative/updateAlternative",// 获取自己系统后台用户信息接口
	    type : "POST",
	    data:JSON.stringify(temp),
	    contentType:"application/json",
	    dataType: "json",
...
function  defineCom(){
   	   ...
   	   list[0]=temp;
		$.ajax({
	    url : ctx+"/common/defineCommonTemplate",// 获取自己系统后台用户信息接口
...
function  updateCom(arr){
	   var id=arr[0];
		...
		$.ajax({
	    url : ctx+"/common/updateCommonTemplate",// 获取自己系统后台用户信息接口
	    data :JSON.stringify(temp),
	    type : "POST",
	    contentType:"application/json",
	    dataType: "json",
...
$(".yg-download").click(function(){
	window.location.href=ctx+"/salary/staffExport";
})
/**
 * 工資條下載
 * @returns
...
$(".gz-download").click(function(){
	window.location.href=ctx+"/salary/export";
})
/**
 * 初始化员工密码
 * @returns
...
$(".pwd_up").click(function(){
	   var mobile=$(".mobile").val();
		$.ajax({
	    url : ctx+"/salary/updatePwd?userName="+mobile,// 获取自己系统后台用户信息接口
	    type : "get",
	    async :true,
	    contentType:"application/json",
	    dataType: "json",
