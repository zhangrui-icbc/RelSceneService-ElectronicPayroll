...
public interface SalaryWebUserMapper{
	...
	SalaryUser login(@Param("username") String username, @Param("password")String password);

	void resetPassword(@Param("username") String username,  @Param("password1") String newPassword1);

	void saveUserKey(@Param("username")String username,@Param("userKey") String userKey);
